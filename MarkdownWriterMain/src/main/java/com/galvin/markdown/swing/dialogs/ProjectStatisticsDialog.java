    ...
    extends JDialog
{
    ...
    private JLabel characterLabel = new JLabel( messages.dialogStatisticsCharacters() );
    private JLabel wordsLabel = new JLabel( messages.dialogStatisticsWords() );
    private JLabel pagesLabel = new JLabel( messages.dialogStatisticsPages() );
    private JLabel charactersPerWordLabel = new JLabel( messages.dialogStatisticsCharactersPerWord() );
    private JTextField charactersPerWordField = new JTextField();
    private JLabel wordsPerPageLabel = new JLabel( messages.dialogStatisticsWordsPerPage() );
    private JTextField wordsPerPageField = new JTextField();
    private JButton doneButton = new JButton( messages.dialogStatisticsDone() );
    private JButton refreshButton = new JButton( messages.dialogStatisticsRefresh() );
    ...
    public ProjectStatisticsDialog( Controller controller )
    {
        ...
        setTitle( messages.dialogStatisticsTitle() );
    ...
    public void refresh()
    {
        try
        {
            ...
            characterLabel.setText( messages.dialogStatisticsCharacters() + " " + Utils.NUMBER_FORMATTER.format( characters ) );
            
            int cpw = getCharactersPerWord();
            long words = Math.max( 1, characters / cpw );
            wordsLabel.setText( messages.dialogStatisticsWords() + " " + Utils.NUMBER_FORMATTER.format( words ) );
            
            int wpp = getWordsPerPage();
            long pages = Math.max( 1, words / wpp );
            pagesLabel.setText( messages.dialogStatisticsPages() + " " + Utils.NUMBER_FORMATTER.format( pages ) );
